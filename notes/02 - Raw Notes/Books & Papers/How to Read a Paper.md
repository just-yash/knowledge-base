
Type : #RawNote #paper            
Date :  2026-05-09   
Tags :  [[paper]]        
Source : [How to read a paper : S. Keshav](https://web.stanford.edu/class/ee384m/Handouts/HowtoReadPaper.pdf#:~:text=1,of%20an%20existing%20system%3F%20A)   
~ ***Yash Agrawall*** ~   

---
# How to Read a Paper
>Three-pass Method 

- Reasons to read paper : 
	- conference or a class
	- keep current in their field 
	- literature survey of a new field 
- Typical researcher spends hundreds of hours every year reading papers 
- Read the paper in up to three passes 
	- [[#1st Pass]] : general idea 
	- [[#2nd Pass]] : grasp paper’s content 
	- [[#3rd Pass]] : paper in depth

---
## 1st Pass

- Quick scan 
- Get a bird’s eye view of the paper 
- You can decide whether you need more passes 
- 5 to 10 minutes 
- Steps 
	1. Read the title, abstract and introduction 
	2. Read the section and sub-section headings, ignore everything else 
	3. Read the conclusions 
	4. Glance over the references : mentally ticking off the ones you’ve already read 
- Qs you should be able to answer after the 1st Pass : the five C’s 
	1. Category : What type of paper is it? 
		- A measurement paper?
		- An analysis of an existing system?
		- A description of a research prototype? 
	2. Context : 
		- Which other papers is it related to? 
		- Which theoretical bases were used to analyze the problem? 
	3. Correctness : 
		- Do the assumptions appear to be valid? 
	4. Contributions : 
		- What are the paper’s main contributions? 
	5. Clarity : 
		- Is the paper well written? 
- Use this information, to decide if you want to read further or not 
	- Maybe it doesn't interest you 
	- You don't know enough about the area to understand the paper 
	- Author’s take invalid assumptions 

> When you write : 
>- Choose coherent section and sub-section titles 
>- write concise and comprehensive abstracts 

---
## 2nd Pass 

- Read with greater care 
- ignore details such as proofs 
- helps to jot down the key points 
- make comments in the margins as you read 
- Steps : 
	1. Look carefully at the figures, diagrams and other illustrations 
		- Special attention to graphs 
			- Are the axes properly labeled 
			- Are results shown with error bars → conclusions are statistically significant 
		- Common mistakes like these separate rushed, shoddy work from the truly excellent 
	2. Mark relevant unread references for further reading 
		- good way to learn about the background of the paper 
- 1 hour 
- You should be able to grasp the content of the paper after this stage 
- Be able to summarize the main thrust of the paper with supporting evidence to someone else 
- This level of detail is appropriate for a paper in which you are interested but doesn't lie in your research speciality 
- Sometimes you still don't understand the paper 
	- Subject matter is new to you → unfamiliar Terminology and acronyms 
	- Author may use a proof or technique you don't understand 
	- poorly written paper → unsubstantiated assertions and numerous forward references
	- late night tired reading 
- Options : 
	1) set paper aside : hoping the paper is irrelevant to your career 
	2) return to the paper later : perhaps are reading background material 
	3) persevere and go on to the 3rd Pass 

---
## 3rd Pass

- Key : Attempt to re-implement the paper 
	- making the same assumptions at the authors 
	- re-create the work 
- Compare this re-creation with the actual paper 
- Identify paper’s innovations and its hidden failings and assumptions 
- 3rd pass requires great attention to detail 
- Identify and challenge every assumption in every statement 
- Think about how you yourself would present a particular idea 
	- sharp insight into the proof and presentation techniques in the paper 
	- add this to your repertoire of tools 
- Jot down ideas for future work 
- Time : 
	- 4-5 hours for beginners 
	- 1-2 hour for experts 
- At the end of this pass : 
	- reconstruct the entire structure of the paper from memory 
	- identify strong and week points 
	- pinpoint 
		- implicit assumptions 
		- missing citations to meaningful work
		- potential issues with experimental or analytical techniques 

---
# Literature Survey 

- Testing paper reading skills → doing a literature survey 
- Read tens of papers (better if unfamiliar field)

## What papers to read? 

1. Academic Search Engine : [Google Scholar](https://scholar.google.com/) 
2. search using some well-chosen keywords to find 3 to 5 recent papers in the area 
3. Do [[#1st Pass]] on each paper to get a sense of the work 
4. Read their related work sections 
5. if you will get a thumbnail summary of the recent survey paper → Read it 
6. If not, Find shared citations and repeated author names in the bibliography 
7. These are key papers and researchers in that area 
8. Download the key papers and set them aside 
9. Go to the websites of the key researchers and see where they’ve published recently 
	1. this will help you identify the top conferences in that field 
	2. best researchers publish in the top conferences 
10. Go to the website for these top conferences and look through their recent proceedings 
11. A quick scan → identify recent high quality related work 
12. First version of the survey : these papers + key papers you set aside earlier 
13. Make two passes through these papers 
14. Find a key paper that you failed to find earlier  

---
# Related Work 

---
## Reading to Review 

1. Timothy Roscoe’s : “Writing reviews for systems conferences”

## Reading to write a Technical paper 

1. Henning Schulzrinne’s comprehensive web site 
2. George Whiteside’s Excellent overview of the process 

## Entire Spectrum of Research Skills 

1. Simon Peyton Jones’ website 

