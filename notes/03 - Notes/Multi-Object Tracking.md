...
- MOT is a graph matching problem; tracklets are nodes, associations are edges → [[Graph Theory]]
- Kalman Filter is Bayesian state estimation → [[Baye's Theorem]]
- Hungarian Algorithm is combinatorial optimization → [[Algorithm Basics]]
- Motion prediction is probabilistic → [[Probability Introduction]]
