...
- Sets are usually denoted by uppercase letters ($A, B, C, \dots$).
- Elements of the Sets are denoted by lowercase letters ($a, b, c, \dots$).
...
###### d) if after finite number of union result is infinite set, then at least one of the input set(disjoint) is infinite (T / F)
...
- FaceNet learns set relationships (identity classes) not boundaries → [[FaceNet]]
- Relational algebra is grounded in set operations → [[Relational Model]]
- Set membership and power sets connect to → [[Discrete Mathematics Introduction]]
- Probability sample spaces are sets → [[Probability Introduction]]
